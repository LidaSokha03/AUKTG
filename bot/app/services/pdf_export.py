...
def cv_to_pdf(cv_obj: CV, out_path:Path = Path('exports')) -> Path:
    ...
    # Підрозділ: персональні дані
    pdf.set_font("Times", "B", 14)
    pdf.cell(0, 10, "Personal Information:", ln=True)
    pdf.set_font("Times", "", 12)
    pdf.multi_cell(0, 8, 
        f"Name: {cv_obj.firstname} {cv_obj.lastname}\n"
    ...
    pdf.set_font("Times", "", 12)
    for lang, level in cv_obj.languages:
        pdf.cell(0, 8, f"- {lang}: {level}", ln=True)
    pdf.ln(4)
    ...
    pdf.set_font("Times", "", 12)
    pdf.multi_cell(0, 8, cv_obj.projects)
    pdf.ln(10)
...
our_curr = CV("auto_generated", "Lida", "Sokha", "lidasosokha@gmail.com", "380964692379", "Bachelor's Degree in BA, UCU", "8 (int in years)", "for the start only in text but after this list of skills ['skill1', 'skill2']", "text for start but after this list of languages ['language1, level', 'language2, level']", "text")
